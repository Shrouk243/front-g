...
function VitalCard({ label, value, unit, trend, trendDir, status, statusColor, statusBg, accentColor, chartData, linkTo }: VitalCardProps) {
  const { colors } = useApp();
  const trendArrow =
    trendDir === "up"
  ...
  const chartSeries: Array<HealthChartSeries<(typeof chartData)[number]>> = [{
    ...
    label,
    color: accentColor,
    latestColor: statusColor,
    warningColor: "#F59E0B",
    criticalColor: "#EF4444",
    thresholds: getDashboardThresholds(label),
  }];

  return (
    <Link to={linkTo} style={{ textDecoration: "none" }}>
      <div style={{
        background: `linear-gradient(180deg, ${colors.cardBg} 0%, ${colors.pageBg} 150%)`,
        borderRadius: 24,
        padding: "18px",
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
          <div>
            <p style={{ fontSize: 11, fontWeight: 700, color: colors.textMuted, margin: "0 0 8px", letterSpacing: "0.08em", textTransform: "uppercase" }}>{label}</p>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 8px", borderRadius: 999, background: statusBg }}>
            {trendArrow}
            <span style={{ fontSize: 10, fontWeight: 800, color: statusColor, letterSpacing: "0.04em", textTransform: "uppercase" }}>{status}</span>
          </div>
        </div>
        ...
        <div style={{ background: colors.cardBg, borderRadius: 18, padding: "12px 10px 12px", border: `1px solid ${colors.borderLight}`, minHeight: 124 }}>
          <HealthBarChart
            ...
            xTickFormatter={(tick) => formatChartTimeLabel(String(tick))}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 12, color: colors.textSecondary, fontWeight: 600, lineHeight: 1.5 }}>{trend}</span>
...
function getTrendDirection(vital: BackendVital) {
  return vital.status === "critical" || vital.status === "elevated" ? "up" : "flat";
}

function buildVitalCard(vital: BackendVital, recentVitals: BackendVital[]): VitalCardData {
  const scoped = recentVitals
    .filter((item) => item.type === vital.type)
    .slice(0, 7)
    .reverse()
    .map((item, index) => {
  ...
  return {
    ...
    value: vital.type === "blood_pressure" && vital.systolic !== null && vital.diastolic !== null ? `${vital.systolic}/${vital.diastolic}` : vital.value,
    unit: getVitalUnit(vital.type),
    trend: `Latest reading on ${lastMeasured}`,
    trendDir: getTrendDirection(vital),
    status: vital.status,
    statusColor,
    statusBg,
    accentColor,
...
export function DashboardPage() {
  ...
  const [userName, setUserName] = React.useState<string>(() => {
    const fullName = getStoredProfile().name || "User";
  ...
  const loadDashboard = React.useCallback(async () => {
    ...
    const score = response.analysis_preview?.health_score ?? response.latest_analysis?.health_score ?? 0;
  ...
  const logReadingButtonText = isAR ? "تسجيل قراءة" : "Log Reading";

  const displayScore = healthScore ?? 0;

  return (
    <div style={{ padding: "24px", maxWidth: 1200, margin: "0 auto" }}>
      <ManualReadingDialog open={dialogOpen} onClose={() => setDialogOpen(false)} onSaved={loadDashboard} />

      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div>
          <p>
  {new Date().toLocaleDateString(t("lang_label") === "العربية" ? "en-US" : "ar-EG", {
    weekday: "long",
    year: "numeric",
    month: "long",
    day: "numeric",
        ...
        <div style={{ background: "#0F2A5C", borderRadius: 20, padding: "20px 24px", display: "flex", alignItems: "center", gap: 16, position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ flex: 1, position: "relative", zIndex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
              ...
              <span style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" }}>{t("dash_overallStatus")}</span>
            </div>
            <p style={{ color: "white", fontSize: 18, fontWeight: 800, margin: "0 0 8px", letterSpacing: "-0.02em" }}>{t("dash_healthLooksGood")}</p>
          </div>
        ...
        <div style={{ background: colors.cardBg, borderRadius: 20, padding: "18px 20px", display: "flex", gap: 14, alignItems: "flex-start", border: `1.5px solid ${colors.border}`, boxShadow: "0 2px 8px rgba(15,31,61,0.04)" }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
              ...
              <span style={{ fontSize: 11, color: colors.textMuted, fontWeight: 500 }}>mock</span>
            </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        ...
        <span style={{ fontSize: 12, color: colors.textMuted, fontWeight: 500 }}>{currentVitals.length ? "Backend synced" : "Waiting for readings"}</span>
      </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
        ...
        {[
          ...
          <Link key={action.path} to={action.path} style={{ flex: "1 1 140px", padding: "14px 10px", borderRadius: 14, background: action.bg, color: action.color, fontSize: 14, fontWeight: 700, textDecoration: "none", textAlign: "center" }}>
            {t(action.labelKey)}
          </Link>
        ))}
      </div>
