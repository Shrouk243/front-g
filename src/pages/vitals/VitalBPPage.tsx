...
function getBPStatus(sys: number, dia: number): { label: string; color: string; bg: string; dot: string } {
  if (sys >= 180 || dia >= 120)
    return { label: "Emergency",     color: "#DC2626", bg: "#FEE2E2", dot: "#DC2626" };
  if (sys >= 140 || dia >= 90)
    return { label: "Very High",     color: "#B91C1C", bg: "#FECACA", dot: "#EF4444" };
  if (sys >= 130 || dia >= 80)
    return { label: "Elevated",      color: "#B45309", bg: "#FEF3C7", dot: "#F59E0B" };
  if (sys >= 120)
    return { label: "Slightly High", color: "#A16207", bg: "#FEF9C3", dot: "#EAB308" };
  if (sys < 90 || dia < 60)
    return { label: "Low",           color: "#1D4ED8", bg: "#DBEAFE", dot: "#3B82F6" };
  return   { label: "Normal",        color: "#16A34A", bg: "#DCFCE7", dot: "#22C55E" };
}
      ...
      {latest && (
        <>
            ...
            <div style={{ background: colors.cardBg, borderRadius: 24, padding: "24px", boxShadow: "0 4px 16px rgba(15,31,61,0.08)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 20 }}>
                ...
                <div>
                  ...
                  {(() => {
                    const bp = getBPStatus(latest.systolic ?? 0, latest.diastolic ?? 0);
                    return (
                      <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: bp.bg, borderRadius: 10, padding: "5px 12px" }}>
                        <div style={{ width: 7, height: 7, borderRadius: "50%", background: bp.dot }} />
          ...
          <div style={{ background: `linear-gradient(180deg, ${colors.cardBg} 0%, ${colors.pageBg} 180%)`, borderRadius: 24, padding: "22px", marginBottom: 16, boxShadow: "0 14px 32px rgba(2,6,23,0.14)", border: `1px solid ${colors.border}` }}>
            ...
            <div style={{ background: colors.cardBg, borderRadius: 20, padding: "16px 14px 10px", border: `1px solid ${colors.borderLight}` }}>
              <HealthBarChart
                data={readings}
                series={[
                  { key: "sys", label: t("history_systolic"), color: "#F59E0B", latestColor: "#FB923C", warningColor: "#F97316", criticalColor: "#EF4444", thresholds: { direction: "high", warning: 130, critical: 140 } },
          ...
          <div style={{ background: colors.cardBg, borderRadius: 18, overflow: "hidden", boxShadow: "0 2px 8px rgba(15,31,61,0.06)" }}>
            {log.map((entry, index) => {
              const bp = getBPStatus(entry.systolic ?? 0, entry.diastolic ?? 0);
              return (
                <div key={entry.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 18px", borderBottom: index < log.length - 1 ? `1px solid ${colors.borderLight}` : "none" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    ...
                    <div style={{ padding: "4px 10px", borderRadius: 6, background: bp.bg }}>
                      <span style={{ fontSize: 12, fontWeight: 700, color: bp.color }}>{bp.label}</span>
                    </div>
                  </div>
                </div>
              );
