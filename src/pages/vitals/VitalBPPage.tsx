...
export function VitalBPPage() {
  ...
  const readings = items.slice(0, 8).reverse().map((item, index) => ({
    time: item.measured_at
      ? new Date(item.measured_at).toLocaleTimeString(isAR ? "ar-EG" : "en-US", {
          hour: "2-digit",
          minute: "2-digit",
        })
      : `#${index + 1}`,
  ...
  const currentStatus = latest ? calculateBPStatus(latest.systolic, latest.diastolic) : "normal";
    ...
    <div style={{ padding: "24px", maxWidth: 900, margin: "0 auto", direction: isAR ? "rtl" : "ltr" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 13, color: colors.textMuted, margin: 0, fontWeight: 500 }}>
            {t("vitals_bpUnit")}
          </p>
        </div>
        <button
      ...
      {latest && (
        <>
          ...
          >
            ...
            >
                ...
                <div>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 6 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column" }}>
                      ...
                      <div
                        style={{ width: 20, height: 1.5, background: colors.textMuted, margin: "4px 0" }}
                      />
                      <span style={{ fontSize: 14, fontWeight: 700, color: "#1A6BCC" }}>
              ...
              <p style={{ fontSize: 12, color: colors.textMuted, margin: 0 }}>
                {isAR ? "آخر قراءة: " : "Latest reading: "}
                {formatVitalTimestamp(latest.measured_at)}
              </p>
            </div>
          </div>
          ...
          <div
            style={{
              background: `linear-gradient(180deg, ${colors.cardBg} 0%, ${colors.pageBg} 180%)`,
              borderRadius: 24,
              padding: "22px",
              marginBottom: 16,
              boxShadow: "0 14px 32px rgba(2,6,23,0.14)",
              border: `1px solid ${colors.border}`,
            }}
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div>
                <span style={{ fontSize: 16, fontWeight: 800, color: colors.textPrimary, display: "block" }}>
                  {t("vitals_todaysTrend")}
                </span>
                <span style={{ fontSize: 12, color: colors.textMuted, fontWeight: 600 }}>
                  {isAR
                    ? "أحدث قراءات ضغط الدم"
                    : "Latest backend readings for systolic and diastolic values"}
                </span>
              </div>
            </div>
            <div
            ...
            >
              <HealthBarChart
                ...
                series={[
                  {
                    key: "sys",
                    label: t("history_systolic"),
                    color: "#F59E0B",
                    latestColor: "#FB923C",
                    warningColor: "#F97316",
                    criticalColor: "#EF4444",
                    thresholds: { direction: "high", warning: 120, critical: 140 },
                  },
                  {
                    key: "dia",
                    label: t("history_diastolic"),
                    color: "#38BDF8",
                    latestColor: "#1D4ED8",
                    warningColor: "#F59E0B",
                    criticalColor: "#EF4444",
                    thresholds: { direction: "high", warning: 80, critical: 90 },
                  },
                ]}
                height={230}
                unit="mmHg"
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      alignItems: "center",
                      padding: "14px 18px",
                      borderBottom: index < log.length - 1 ? `1px solid ${colors.borderLight}` : "none",
                    }}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <span style={{ fontSize: 14, color: colors.textPrimary, fontWeight: 600 }}>
                        {formatVitalTimestamp(entry.measured_at)}
                      </span>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                      <div>
                        <span style={{ fontSize: 17, fontWeight: 800, color: "#F59E0B" }}>{entry.systolic}</span>
                        <span style={{ fontSize: 14, color: colors.textMuted, fontWeight: 500, margin: "0 4px" }}>
                          /
                        </span>
                        ...
                        <span
                          style={{
                            fontSize: 12,
                            color: colors.textMuted,
                            ...(isAR ? { marginRight: 4 } : { marginLeft: 4 }),
                          }}
                        >
                          mmHg
                        </span>
                      </div>
                      <div style={{ padding: "4px 10px", borderRadius: 6, background: sLog.bg }}>
                        <span style={{ fontSize: 12, fontWeight: 700, color: sLog.color }}>{sLog.label}</span>
                      </div>
                    </div>
                  </div>
                );
