...
function getHRStatus(bpm: number): { status: "normal" | "elevated" | "critical"; label: string; bg: string; dot: string; text: string } {
  if (bpm < 50 || bpm > 170) {
    return { status: "critical", label: "Critical", bg: "#FEE2E2", dot: "#EF4444", text: "#DC2626" };
  }
  if (bpm > 100) {
    return { status: "elevated", label: "Elevated", bg: "#FEF3C7", dot: "#F59E0B", text: "#B45309" };
  }
  return { status: "normal", label: "Normal", bg: "#DCFCE7", dot: "#22C55E", text: "#16A34A" };
}
export function VitalHeartRatePage() {
  ...
  return (
    <div style={{ padding: "24px", maxWidth: 900, margin: "0 auto" }}>
      <ManualReadingDialog open={dialogOpen} onClose={() => setDialogOpen(false)} onSaved={loadReadings} initialType="heart_rate" allowTypeSelection={false} />
        ...
        <>
            ...
            <div style={{ background: colors.cardBg, borderRadius: 24, padding: "24px", boxShadow: "0 4px 16px rgba(15,31,61,0.08)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 16 }}>
                ...
                <div>
                  ...
                  {(() => {
                    const hr = getHRStatus(getVitalNumericValue(latest));
                    return (
                      <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: hr.bg, borderRadius: 12, padding: "5px 12px", marginTop: 6 }}>
                        <div style={{ width: 7, height: 7, borderRadius: "50%", background: hr.dot }} />
                        <span style={{ fontSize: 13, fontWeight: 700, color: hr.text }}>{hr.label}</span>
                      </div>
                    );
                  })()}
                </div>
          ...
          <div style={{ background: `linear-gradient(180deg, ${colors.cardBg} 0%, ${colors.pageBg} 180%)`, borderRadius: 24, padding: "22px", boxShadow: "0 14px 32px rgba(2,6,23,0.14)", border: `1px solid ${colors.border}` }}>
            <div style={{ marginBottom: 16 }}>
              ...
              <span style={{ fontSize: 12, color: colors.textMuted, fontWeight: 600 }}>Latest backend heart-rate readings</span>
            </div>
            <div style={{ background: colors.cardBg, borderRadius: 20, padding: "16px 14px 10px", border: `1px solid ${colors.borderLight}` }}>
              <HealthBarChart
                data={readings}
                series={[{ key: "value", label: t("vital_hr"), color: "#FB7185", latestColor: "#EF4444", warningColor: "#F59E0B", criticalColor: "#DC2626", thresholds: { direction: "high", warning: 100, critical: 170 } }]}
                height={220}
                unit="bpm"
                yDomain={[40, 180]}
