...
function getO2Status(spo2: number): { label: string; color: string; bg: string; dot: string } {
  if (spo2 < 90)
    return { label: "Low — Concerning", color: "#DC2626", bg: "#FEE2E2", dot: "#EF4444" };
  if (spo2 < 95)
    return { label: "Slightly Low",     color: "#B45309", bg: "#FEF3C7", dot: "#F59E0B" };
        ...
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16, marginBottom: 16 }}>
            <div style={{ background: colors.cardBg, borderRadius: 24, padding: "28px", boxShadow: "0 4px 16px rgba(15,31,61,0.08)", display: "flex", flexDirection: "column", alignItems: "center" }}>
              ...
              {(() => {
                const o2 = getO2Status(getVitalNumericValue(latest));
                return (
                  <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: o2.bg, borderRadius: 12, padding: "5px 14px", marginBottom: 10 }}>
                    <div style={{ width: 7, height: 7, borderRadius: "50%", background: o2.dot }} />
                    <span style={{ fontSize: 13, fontWeight: 700, color: o2.color }}>{o2.label}</span>
                  </div>
                );
              })()}
